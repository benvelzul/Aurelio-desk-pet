...
# VIBRATION / MOVEMENT
# ─────────────────────────────────────────
def handle_movement(now):
    if vibration.value() == 1:   # sensor triggers HIGH
        state["vibration_count"] += 1
        state["last_vibration"]   = now
        
        # start new window if not already tracking
        if state["vibe_window_start"] == 0:
            state["vibe_window_start"] = now

    # window expired — evaluate what happened
    if (state["vibe_window_start"] > 0
            and elapsed(state["vibe_window_start"]) > VIBE_WINDOW):
        
        hits = state["vibration_count"]
        print(f"[VIBE] window ended — hits={hits}")

        if hits >= VIBE_THRESHOLD:
            on_picked_up(now)

        # reset window
        state["vibration_count"]   = 0
        state["vibe_window_start"] = 0

def on_picked_up(now):
    """React to being picked up based on current mood."""
    print("[VIBE] picked up!")

    if state["is_sleeping"]:
        # woken up by movement — annoyed
        wake_up(now)
        trigger_annoy(now, "vibe_wake")
        print("[VIBE] sleeping -> angry")

    elif state["is_sleepy"]:
        # disturbed while drowsy — mildly annoyed
        wake_up(now)
        state["anger_level"] = 1
        state["annoy_end"]   = now + ANNOY_DURATION
        start_shake(3, 200)
        print("[VIBE] sleepy -> mild angry")

    elif state["is_sad"]:
        # picked up while sad — cheers up
        state["is_sad"]    = False
        state["is_happy"]  = True
        state["happy_end"] = now + random.randint(4000, 7000)
        state["last_interaction"] = now
        print("[VIBE] sad -> happy")

    elif state["anger_level"] > 0:
        # already angry — gets more angry
        trigger_annoy(now, "vibe_angry")
