...
# HARDWARE
# ─────────────────────────────────────────
# OLED display on I2C channel 0 (Pins: SDA=0, SCL=1)
i2c = I2C(0, scl=Pin(1), sda=Pin(0))
oled = ssd1306.SSD1306_I2C(128, 64, i2c)

# ADXL345 Accelerometer on I2C channel 1 (Pins: SDA=6, SCL=7)
i2c_accel = I2C(1, scl=Pin(7), sda=Pin(6), freq=400000)

# Wake up ADXL345 sensor (Power Control Register 0x2D -> Measure Mode 0x08)
try:
    i2c_accel.writeto(0x53, b"\x2D\x08")
    print("[ACCEL] ADXL345 Initialized Successfully")
except OSError:
    print("[ACCEL] ERROR: ADXL345 not found on I2C channel 1")
...
# ACCELEROMETER
# ─────────────────────────────────────────
def handle_accelerometer(now):
    """ Reads raw data from ADXL345 and prints it for Thonny Plotter """
    try:
        # Request 6 bytes starting from data register 0x32
        i2c_accel.writeto(0x53, b"\x32")
        d = i2c_accel.readfrom(0x53, 6)

        # Convert raw bytes into X, Y, Z signed integers
        ...
        # Print layout matches the Thonny Plotter perfectly
        print(f"({x}, {y}, {z})")
        
        # 💡 Future pickup logic outline:
        # if abs(x) > THRESHOLD or abs(y) > THRESHOLD:
        #     wake_up(now)

    except OSError:
        # Gracefully handle disconnected/loose wires without crashing the robot
        pass
...
def render(sx=0, sy=0):
    ...
    if state["anger_level"] > 0:
        # Avoid clashing with the tuple output in Thonny plotter
        # print("annoyed") 
        annoyed_face(sx, sy)
...
while True:
    ...
    # Read and output accelerometer tuple
    handle_accelerometer(now)
